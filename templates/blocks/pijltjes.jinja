{% if links['link_back'] %}
	...
	{% if links['link_back'] is instance('date') %}
		<a href="{{ links['link_back']|dateformat('ddMMyyyy') }}" alt="{{ links['link_back'] }}">{{ links['link_back'] }}</a>
	{% else %}
...
{% if links['link_forward'] %}
	...
	{% if links['link_forward'] is instance('date') %}
		<a href="{{ links['link_forward']|dateformat('ddMMyyyy') }}" alt="{{ links['link_forward'] }}">{{ links['link_forward'] }}</a>
	{% else %}
